...
const menuList = [
  ...
  },
  {
    ...
    link: [
      '/admin/admin-product-mgt-page',
      '/admin/admin-product-add-form',
      '/admin/admin-product-edit-form',
    ],
              ...
              <Link key={el.id} to={el.link[0]}>
                {el.link.includes(location.pathname) ? (
                  <Button
                    ...
                    color='red'
                  >
                    <>{cloneElement(el.component, { color: '#D2001E' })}</>{' '}
                    {el.name}
                  </Button>
                ) : (
                  <Button textPosition='start' bg='gray' width='full'>
                    {el.component} {el.name}
                  </Button>
                )}
              </Link>
            ) : (
